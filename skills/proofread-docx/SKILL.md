...
python ${SKILL_ROOT}/scripts/filter_glossary_violations.py \
  --issues <issues.json> --pdf <原文PDF> --docx <译文DOCX> \
  --output <cleaned.json> --auto-remove
...
| 产物 | 命名模式 | 示例 |
|------|---------|------|
| 校对稿（含批注） | `<原文前缀>_校对稿.docx` | `[1]-CN_校对稿.docx` |
| 校对报告 | `<原文前缀>_校对报告.docx` | `[1]-CN_校对报告.docx` |
| 中间产物 | 放版本目录 `v{N}/`，不编码进文件名 | `proofread/1/v16/` |

> **原则**：版本用目录区分（`v1/` `v2/` …），文件名保持稳定。不要出现 `_v6` `_clean` `_final` 等临时后缀。
...
python ${SKILL_ROOT}/scripts/write_report.py \
  ...
  --mechanical-report "<Phase 3 输出>" \  # 必传：缺则术语库匹配率显示"无术语库"
...
| write_report | 术语库匹配率显示"无术语库" | ✅ 传 `--mechanical-report` 参数，指向 Phase 3 输出 JSON |
