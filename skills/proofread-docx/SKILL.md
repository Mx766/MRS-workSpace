...
```
Step 4.0.1: 从 cache/split_target.json 读取全部段落（148 段）
Step 4.0.2: 按 15 段一批切分，生成批次清单（10 批）
Step 4.0.3: 逐批派发 Agent（model=fable），执行完整 34 项检查清单
Step 4.0.4: 每批完成后合并 issues，更新 cache/phase4_coverage.json
Step 4.0.5: 全部批次完成后，覆盖率 100% → 进入 Phase 5
```
