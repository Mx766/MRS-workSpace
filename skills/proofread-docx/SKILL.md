...
| # | 校验项 | 阻断条件 |
|---|--------|----------|
| V1 | 顶层结构 | issues_phase4.json 不是 dict 格式（纯列表被拒绝） |
| V2 | dimension_coverage | 缺少检查项、checked=false 的维度 ≥ 3 |
| V3 | paragraph_coverage | 覆盖率 < 100%、全文档 has_issue 段落数 = 0 |
| V4 | issues | 缺少必填字段、severity/justification/confidence 值非法 |
| V5 | phase3_verdicts | 缺少此字段、Phase 3 发现未被全部覆盖、explanation < 20 中文字 |
...
**铁律 E（脚本化）**：v2.11 的铁律 E（禁止零 issue 批次）现在由 V3 校验自动执行——`has_issue` 段落数 = 0 直接阻断。
