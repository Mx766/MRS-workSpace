...
Phase 3.5: 上下文注入（脚本）← v2.14 新增
	   ↓
Phase 3.6: 判决书生成（脚本）← v2.15 新增
	   ↓
Phase 4: 逐章校对（Agent）← 最核心，耗时最长
	   ↓
Phase 4.35: 输出校验（脚本）← v2.15 新增（硬阻断）
	   ↓
...
### 0.6 缓存隔离——每次运行全量重建（铁律 7）`v2.16`

> **背景**：Agent 在两篇文档同时跑时倾向于复用旧版本 `cache/` 目录下的中间产物——直接拷贝而非重新执行脚本。这导致 Phase 3 数据过期、Phase 4 AI 输出被复用、整个 pipeline 形同虚设。

**铁律：每次运行从零开始，所有中间产物全量重建。**

| 规则 | 说明 |
|------|------|
| **新版本 = 新 cache** | `proofread/<项目>/v<N>/` 下的 `cache/` 目录必须为空开始。Phase 0-3 全部脚本必须重新执行 |
| **禁止跨版本拷贝** | 禁止从 `v1/cache/` 拷贝任何文件到 `v2/cache/`。包括但不限于：`split_*.json`, `issues_mechanical*.json`, `phase4_context.json`, `phase3_verdict_sheet.json`, `batch_*_prompt.json` |
| **禁止跳过脚本** | 即使"上次跑过同样的文档"，也必须重新执行 Phase 0→3 全部脚本。脚本是确定性的，但输入文件可能已变化（术语库更新、SKILL 版本升级） |
| **Phase 4 严禁复用** | `issues_phase4*.json` 是 AI 产出，换模型/换版本后必须重审。禁止从旧版本目录拷贝 Phase 4 输出 |
| **例外** | `glossary.json`（术语库本身，非脚本产出）可跨版本引用。`pair_info.json` 仅含文件配对信息，可复用但建议重建 |

**Agent 自查**——进入 Phase 4 之前必须确认：

```
□ cache/ 目录下所有文件均为本次运行产出（检查文件修改时间）
□ 未从任何旧版本 v<N>/cache/ 拷贝文件
□ Phase 0→3.6 全部脚本均已在本次运行中执行（非"上次已跑过"）
□ 术语库文件（glossary.json）路径指向 skill 共享目录而非版本 cache
```

**违规检测**：Phase 4.35 校验脚本会检查 `issues_phase4.json` 的 `version` 字段——若不是当前 SKILL 版本号，阻断并报"疑似复用旧版本 AI 产出"。
...
□ [ ] 0.6 缓存隔离: 确认 cache/ 为空或仅含本次产出 -> 未从旧版本拷贝: [ ] 全部脚本已重跑: [ ]
```
## Phase 2: 文档拆分
...
**Phase 3.5 之后、Phase 4 之前，必须运行以下脚本**，将 Phase 3 机械检查的每一条 finding 转成 Phase 4 的必答问卷：

```bash
python ${SKILL_ROOT}/scripts/build_phase3_verdicts.py \
  --mechanical cache/issues_mechanical.json \
  --output cache/phase3_verdict_sheet.json
```

**输出 `cache/phase3_verdict_sheet.json` 结构**：
...
```json
{
  ...
  "verdict_requirements": {
    ...
    "must_cover_all": true,
    "note": "每一条 Phase 3 发现都必须有对应的判决。禁止用笼统理由批量 dismiss。"
  },
  "verdicts": [
    {
      "verdict_id": "term_001",
      ...
      "paragraph_index": 16,
      "source_term": "denatured",
      "expected_target": "改性",
      "phase3_judgment": "术语 \"denatured\" 应译为 \"改性\"，但译文中未找到",
      "verdict": null,
      "explanation": null,
      "severity_if_confirmed": null
    }
  ]
}
```
...
### Phase 4 强制任务清单（不可跳过，逐项勾选，未全部打勾 = Phase 4 未完成）`v2.15`

> ⚠️ **此清单是 Phase 4 的"驾驶舱检查单"。每一项必须在你确认完成后显式标记 `[x]`。Phase 4 结束时清单必须全部 `[x]`。任何一项 `[ ]` = 不可进入 Phase 4.35。**

```
Phase 4 前置准备：
□ [ ] 1. 加载 cache/phase4_context.json — 确认领域：[____]，必检项数量：[____]，高风险段落数：[____]
□ [ ] 2. 加载 cache/phase3_verdict_sheet.json — 确认待判决的 Phase 3 发现总数：[____] 条
□ [ ] 3. 从 cache/split_target.json 读取全部段落 — 确认总段落数：[____] 段
□ [ ] 4. 切分批次 — 确认批次数：[____] 批（每批 15 段）

Phase 4 逐批审查（每完成一批立即更新）：
□ [ ] Batch 01: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 02: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 03: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 04: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 05: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 06: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 07: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 08: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 09: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
□ [ ] Batch 10: 段 ____-____  | issues: ____ 条 | verdicts: ____/____ | 审查人: ____
（若分段 >10 批，继续追加行）

Phase 4 合并与校验：
□ [ ] 5. 各批次 dimension_coverage 聚合 — 确认全部 35 个 check_id 已被覆盖
□ [ ] 6. 各批次 paragraph_coverage 聚合 — 确认全部 [____] 段已被覆盖
□ [ ] 7. 各批次 issues 合并写入 cache/issues_phase4.json
□ [ ] 8. phase3_verdicts 完整性自查 — confirmed: ____ 条 / false_positive: ____ 条 / needs_context: ____ 条 / unverifiable: ____ 条（合计 = 待判决总数 ____/____）
□ [ ] 9. 运行 validate_phase4_output.py → 确认 passed = true
```

**铁律**：此清单中的每一项 `□ [ ]` 在 Phase 4 结束时必须是 `☑ [x]`。留空的项 = 未完成 = Phase 4 不通过。禁止说"差不多都做了"就跳过。
...
4. MULTI-ISSUE PER PARAGRAPH (v2.18):
   Finding one issue in a paragraph does NOT mean the paragraph is done.
   After finding ANY issue, you MUST continue checking ALL remaining
   dimensions for that SAME paragraph. One paragraph can have multiple
   independent issues (e.g., terminology + awkward phrasing + missing
   number). The check is COMPLETE only after every dimension has been
...
- [ ] 3.8 **领域惯例一致**（v2.19）：动物实验——"男/女"→"雄/雌"，"病人"→"动物/受试动物"，"摄入/口服"→"给药"（注射途径时）。临床试验——受试者/患者/健康志愿者术语区分。体外实验——培养基/培养液/基质/介质区分
...
- [ ] 4.4 缩写规范：首次出现是否标注"全称(ABBR)"？后续出现是否保持缩写形式一致？不可首次用"全称(ABBR)"后文又变回全称或不同缩写
- [ ] 4.5 跨领域区分：多义术语是否按文档领域选对译法

---

#### 五、格式与表格（7 项）`v2.18 扩展`

> Phase 3 脚本已做机械检查，Agent 复核 + 表格内容翻译检查

- [ ] 5.1 标题层级：H1/H2/H3 对应 + 编号是否一致？
- [ ] 5.2 表格结构：行列数/合并单元格/表头/跨页表头是否与原文一致？
...
- [ ] 5.8 **句中断行/段落结构**（v2.19）：段落中是否有不正常的换行？中文句子是否被错误切成多行？每行结尾是否为完整句或正常断句？PDF提取文本尤需注意
...
### 4.3 输出格式（强制字段 + 覆盖率追踪）`2026-06-30 强化, v2.15 更新`

每批输出必须包含三部分：**A. 覆盖率声明** + **B. 疑点列表** + **C. Phase 3 判决**。缺一不可。

> ⚠️ **v2.16 新增**：每批输出顶层必须包含 `"version": "2.16"` 字段。Phase 4.35 校验器会检查此字段——版本不匹配则视为复用旧版本 AI 产出，直接阻断。
...
**Section C: PHASE3_VERDICTS（Phase 3 发现逐条判决）`v2.15 新增`** — 每条 Phase 3 机械检查发现必须有对应判决：
  ...
  "phase3_verdicts": [
    {
      "verdict_id": "term_001",
      "verdict": "confirmed",
      "explanation": "译文确实未使用术语库要求的译法...≥20 个中文字说明理由",
...
**verdict 枚举**：`confirmed` | `false_positive` | `needs_context` | `unverifiable`
**explanation 要求**：≥ 20 个中文字符，用中文详细说明判断理由。禁止笼统理由（如"跨领域匹配"、"全部假阳性"）批量 dismiss。
**⚠️ v2.16 强制**：每条 verdict 的 explanation **必须不同**。复制粘贴同一段解释到多条 verdict = 校验不通过。每条 explanation 必须引用该条的具体术语/数字，说明为什么在此语境下成立或不成立。

---

### 4.35 输出校验（硬阻断网关）`2026-06-30 新增 v2.15`

> **背景**：v2.14 的 mandatory schema 仅靠 AI prompts 约束——龙虾模型不遵守时下游脚本静默接受残缺数据（只输出段落状态、无 dimension_coverage、无 phase3_verdicts）。v2.15 起加入脚本侧硬校验，格式不合格的输出强制阻断。

**Phase 4 全部批次完成、issues_phase4.json 写完后，必须运行校验脚本**：

```bash
python ${SKILL_ROOT}/scripts/validate_phase4_output.py   --issues cache/issues_phase4.json   --verdict-sheet cache/phase3_verdict_sheet.json   --split-target cache/split_target.json   --phase3-issues cache/issues_mechanical_filtered.json   --output cache/phase4_validation_report.json
```

**校验项目**（任一项不通过 → 阻断，禁止进入 Phase 4.4）：

| # | 校验项 | 阻断条件 | 版本 |
|---|--------|----------|------|
| V1 | 顶层结构 | issues_phase4.json 不是 dict 格式（纯列表被拒绝） | v2.15 |
| V2 | dimension_coverage | 缺少检查项、checked=false 的维度 ≥ 3 | v2.15 |
| V3 | paragraph_coverage | 覆盖率 < 100%、全文档 has_issue 段落数 = 0 | v2.15 |
...
```json
{
  ...
  "errors": [...],
  "warnings": [...],
  "stats": {"total_issues": 0, "total_paragraphs": 148},
  "action_required": "修复以上 error 后再运行 validate_phase4_output.py。"
}
```

**若 `passed = false`**：Agent 必须根据 errors 逐一修复 `issues_phase4.json`，然后重新运行校验，直到 `passed = true` 才能进入 Phase 4.4。禁止跳过此步骤。

**铁律 E（脚本化）**：v2.11 的铁律 E（禁止零 issue 批次）现在由 V3（has_issue 段落数 = 0）和 V7（总 issue 数异常低）双重校验自动执行——空跑直接阻断，不可跳过。
...
□ [ ] 七道校验逐项确认:
    ...
    V4 issues: [ ] pass  [ ] fail
    V5 phase3_verdicts: [ ] pass  [ ] fail（含 confirmed=0 阻断）
    V6 解释唯一性: [ ] pass  [ ] fail（模板化 copy-paste 阻断）← v2.16 新增
    V7 issue 数量: [ ] pass  [ ] fail（0/1 issue 阻断）← v2.16 新增
□ [ ] ⚠️ 若 V5/V6/V7 任意一项 fail → AI 敷衍 → 必须换模型重审，不可原模型重试
```
...
python ${SKILL_ROOT}/scripts/calibrate_severity.py \
  ...
  --context cache/phase4_context.json \
  --phase3-issues cache/issues_mechanical_filtered.json \
