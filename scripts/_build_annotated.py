#!/usr/bin/env python3
"""Build annotated DOCX: python-docx for highlights, ZIP/lxml for comment refs + comments.xml.

KEY INSIGHT: Never mix lxml elements into python-docx's live XML tree.
python-docx's OxmlElement and raw lxml etree.Element use different namespace
representations; mixing them corrupts the saved file.

CRITICAL: For paragraphs with multiple issues, commentRangeStart elements must
be in the same ID order as commentRangeEnd elements. Otherwise Word sees
crossing ranges and refuses to open the file.
"""
...
with open('cache/issues_phase4.json', 'r', encoding='utf-8') as f:
    issues = json.load(f).get('issues', [])

doc = Document('proofread/未校对、未排版_CHN.docx')
hl_colors = {'critical': ('FFCCCC', 'red'), 'medium': ('FFF9C4', 'yellow'), 'low': ('C8E6C9', 'green')}
sev_order = {'critical': 0, 'medium': 1, 'low': 2}

# ═══════════════════════════════════════════════════════════════
# STEP 1: python-docx adds HIGHLIGHTS ONLY (OxmlElement, no lxml)
# ═══════════════════════════════════════════════════════════════
        ...
        if target:
            for run in para.runs:
                if not run.text.strip():
                    continue
                if target[:40] in run.text or run.text in target:
...
# ═══════════════════════════════════════════════════════════════
# STEP 2: Save via python-docx → temp file (highlights preserved)
# ═══════════════════════════════════════════════════════════════
...
for para_idx in sorted(issues_by_para.keys()):
    if para_idx > len(paragraphs):
        continue
    para_elem = paragraphs[para_idx - 1]  # 0-based index into XML

    sorted_issues = sorted(issues_by_para[para_idx],
                           key=lambda x: sev_order.get(x.get('severity', 'medium'), 1))

    # ── First pass: assign comment IDs, build comment XML entries ──
    para_comments = []  # (comment_id, severity, severity_label, issue_dict)
    for issue in sorted_issues:
        sev = issue.get('severity', 'medium')
        sev_label = {
            'critical': '🔴 严重（必须修改）',
    ...
    # ── Insert all commentRangeStart elements IN ID ORDER ──
    # Find the insertion reference point: after pPr, or at start
    pPr = para_elem.find(f'{{{NS_w}}}pPr')
    insert_after = pPr  # None if no pPr
    ...
    for cid, sev, sev_label, issue in para_comments:
        ...
        insert_after = crs  # next crs goes after this one (preserves ID order!)

    # ── Append all commentRangeEnd elements IN ID ORDER ──
    for cid, sev, sev_label, issue in para_comments:
        cre = etree.Element(f'{{{NS_w}}}commentRangeEnd')
        cre.set(f'{{{NS_w}}}id', str(cid))
        para_elem.append(cre)

    # ── Append all commentReference runs IN ID ORDER ──
    for cid, sev, sev_label, issue in para_comments:
    ...
    # ── Build comment content in comments.xml ──
    for cid, sev, sev_label, issue in para_comments:
        ...
        for i, line in enumerate(lines):
            ...
            if i == 0:
                # First line (severity label) — bold, colored, 9pt
...
# --- 3e: Write final ZIP preserving ZipInfo for unmodified entries ---
import os as _os
_os.makedirs('proofread/v3', exist_ok=True)
output = 'proofread/v3/未校对、未排版_CHN_校对稿v3.docx'
...
# Verify
orig = Document('proofread/未校对、未排版_CHN.docx')
anno = Document(output)
diffs = sum(1 for o, a in zip((p.text for p in orig.paragraphs),
                               (p.text for p in anno.paragraphs)) if o != a)
print(f'OK: {comment_id} comments | {diffs} text diffs | saved to {output}')

# Quick check: verify crs/cre ordering
print('Checking comment range ordering...')
with zipfile.ZipFile(output, 'r') as zf:
    final_doc = etree.fromstring(zf.read('word/document.xml'))
    body = final_doc.find(f'{{{NS_w}}}body')
    for i, p in enumerate(body.findall(f'{{{NS_w}}}p')):
        ...
        if crs_ids != cre_ids:
            print(f'  MISMATCH para {i+1}: crs={crs_ids}, cre={cre_ids}')
print('Done.')
